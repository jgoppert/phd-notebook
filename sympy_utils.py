...
def matrix_to_latex(m, newline=False):
    return mech.mlatex(sympy.Matrix(m))


def ss_to_latex(ss, subscript='', newline=False):
    if newline:
        nl = r'\\'
    else:
        nl = r',\ '
    s = ''
    for name, mat in zip('A B C D'.split(), [ss.A, ss.B, ss.C, ss.D]):
        s += '&{name:s}{subscript:s}='.format(**locals()) \
             + matrix_to_latex(mat) + nl
    return s
